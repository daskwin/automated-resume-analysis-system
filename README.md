...
Проект позволяет загрузить резюме, извлечь из него текст, определить профессиональное направление кандидата, предсказать IT-роль, извлечь ключевые сущности и оценить соответствие резюме добавленным вакансиям.
...
```env
S3_MODEL_BUCKET=mlops-intro-daskw
...
Проект является прототипом и имеет ряд ограничений:

- качество моделей зависит от открытых обучающих наборов данных;

- сервис ориентирован на англоязычные резюме;

- сопоставление резюме с вакансиями является предварительной оценкой и не заменяет экспертное решение HR-специалиста.